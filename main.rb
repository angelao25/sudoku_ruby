class SudokuPuzzle
    ...
    while !complete?
      @board.each_with_index do |row, row_index|
        row.each_with_index do |number, number_index|
          if number.zero?
            # get the list of all already listed values ( horizontal, vertical, 3x3 boxA)
            ...
            # repeat over and over until all of the zeros are gonei
          #end
          end
        end
      end
  ...
  def box_values(row_index, number_index)
    row_indexes = BOX_VALUES[row_index]
    number_indexes = BOX_VALUES[number_index]

    values=[]

    row_indexes.each do |r_index|
      number_indexes.each do |n_index|
